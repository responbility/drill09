...
class Run:
    ...
    def enter(self, e):
        if right_down(e) or left_up(e):
            self.boy.dir, self.boy.face_dir = 1, 1
...
class Sleep:
    ...
    def enter(self, e):  # FIX: event 인자 추가
        self.boy.dir = 0
...
class Boy:
    ...
    def handle_event(self, event):
        ...
        self.RUN = Run(self)  # FIX: RUN 상태 추가
        self.state_machine = StateMachine(
            self.IDLE,
            {
                self.SLEEP: {time_out:self.SLEEP, right_down: self.RUN, left_down: self.RUN, right_up: self.RUN, left_up: self.RUN},
                self.IDLE: {right_up: self.IDLE, left_up: self.IDLE, right_down: self.IDLE, left_down: self.IDLE}
            }
        )